    ...
    def read_csv_file():
        ...
        with open(file_to_open,'r', encoding ="utf8") as this_csv_file:
            ...
            for row in this_csv_reader:
                ...
                ba_bug, ba_dark, ba_dragon, ba_electric = float(row[1]), float(row[2]), float(row[3]), float(row[4])        #battle aganst bug, dark, dragon, and electric types
                ba_fairy, ba_fight, ba_fire, ba_flying = float(row[5]), float(row[6]), float(row[7]), float(row[8])         #battle against fairy, fighting, fire, and flying types
                ba_ghost, ba_grass, ba_ground, ba_ice = float(row[9]), float(row[10]), float(row[11]), float(row[12])       #battle against ghost, grass, ground, and ice types
                ba_normal, ba_poison, ba_psychic, ba_rock = float(row[13]), float(row[14]), float(row[15]), float(row[16])  #battle against normal, poison, psychic, and rock types
                ba_steel, ba_water = float(row[17]), float(row[18])                                                         #battle against steel and water types
                ...
                POKEDEX.append(Pokemon(ba_bug, ba_dark, ba_dragon, ba_electric, ba_fairy, ba_fight, ba_fire, ba_flying,
                         ...
                         ba_steel, ba_water, attack_stat, base_stats, rate, poke_class, defense_stat, needed_exp,
                         poke_height, hp_stat, poke_jp_name, poke_en_name, male_ratio, poke_num, sp_attack_stat,
                         sp_defense_stat, speed_stat, poke_type1, poke_type2, poke_weight, poke_gene, poke_is_legend,
